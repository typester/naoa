# based on MENTA::Template, by tokuhirom, which is
# based on Mojo::Template. Copyright (C) 2010, Sebastian Riedel.

package NanoA::Template;
use strict;
use warnings;
use constant DEBUG => $ENV{NANOA_TEMPLATE_DEBUG} || 0;
...
sub build {
    ...
    for my $line (@{$self->{tree}}) {
        ...
        for (my $j = 0; $j < @{$line}; $j += 2) {
            ...
            # Expression
            if ($type eq 'expr') {
                $lines[-1] .= "\$_NanoA .= escape_html(scalar $value);";
            }
    ...
    # Wrap
    $lines[0] ||= '';
    $lines[0]   = q/sub { my $_NanoA = '';/ . $lines[0];
    $lines[-1] .= q/return $_NanoA; }/;
